...
function Player:init(  )	
	Player.body = World:newBSGRectangleCollider(game.X+100,game.Y+100,10,10,1,{collision_type="Player"})	
...
function Player:update(dt)
	...
	if dx ~= 0  or dy ~= 0 then
		...
		x,y = x+dx, y+dy
		if x<Map.WIDTH * Map.TILE_SIZE and y<Map.HEIGHT * Map.TILE_SIZE and x>=0 and y>=0 then
