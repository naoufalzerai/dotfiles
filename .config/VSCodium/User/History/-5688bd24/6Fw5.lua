...
for i=1,Map.WIDTH do
	...
	for j=1,Map.HEIGHT do
		Map.GROUND[i][j]=math.random(1,2)
	end
end
...
local function loadSprites( source )
	...
	LAYERS_GROUND[2].quad = love.graphics.newQuad(0 , 16, Map.TILE_SIZE, Map.TILE_SIZE,w,h)
...
function Map:updateGround(  )
	...
	for i=_i,i_max,1 do
		for j=_j,j_max,1 do
			...
			if ib ~=0 then
				tilesetBatch:add(LAYERS_GROUND[1].quad, x-300, y-300)
			end
		end
	end
	tilesetBatch:flush()
