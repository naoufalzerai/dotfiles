...
function map.update()
    ...
    for i=1,NBR_BLOCKS_X,1 do
        for j=1,NBR_BLOCKS_Y,1 do            
            tilesetBatch:add(map.level[map.current_layer][i][j], (i-1)*TILE_SIZE, (j-1)*TILE_SIZE)            
        end
    end
...
local function generate(level,layer)
    ...
    for i = 1, NBR_BLOCKS_X, 1 do
        map.level[level][i] = {}
       for j = 1, NBR_BLOCKS_Y, 1 do
            map.level[level][i][j] = room_type.ground[math.random(1,#room_type.ground)]
       end 
    end
end

function map.create(seed)
    loadSprites("sprites/Grass.png")
